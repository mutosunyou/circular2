...
class CircularFlow
{
  ...
  public function createCircular($userArray, $circularTitle, $circularCont, $q1cont, $q1ansArray, $q1biko, $q2cont, $q2ansArray, $q2biko, $path, $secret){
    ...
    if(count($userArray) > 0){
      ...
      $this->mailing($madds, '【回覧通知】'.$circularTitle, '以下の回覧が来ています。'.PHP_EOL.'システムで確認してください。'.PHP_EOL.'http://192.168.100.209/workflow/index.php?goto=/circular/index.php'.PHP_EOL.PHP_EOL.'表題：'.$circularTitle);
      return true;
  ...
  public function showList($state, $isFinished){
    ...
    if (count($result) > 0) {
      ...
      $body .= '<tr>';
      $body .= '<th style="background-color:#555555;color:#FFFFFF;">状態</th><th style="background-color:#555555;color:#FFFFFF;">作成者</th><th style="background-color:#555555;color:#FFFFFF;">回覧開始日</th><th style="background-color:#555555;color:#FFFFFF;">表題</th><th style="background-color:#555555;color:#FFFFFF;">アクション</th>';
      $body .= '</tr>';
    }
    ...
    for ($i=0; $i < count($result); $i++) {
      ...
      if ($answerer == 1 && $checked == 0 && $result[$i]['status'] != 2) {
        $body .= '<tr style="font-weight:bold;">';
        $body .= '<td style="color:#3366FF;">要確認</td>';
      }else{
        $body .= '<tr style="background-color:#cccccc;">';
        $body .= '<td>'.$this->statusFromInt(intval($result[$i]['status'])).'</td>';
      }
      $body .= '<td>'.$this->nameFromUserID(intval($result[$i]['owner'])).'</td>';
      $body .= '<td>'.date('Y/m/d',mySQLDatetimeToUnixTimestamp($result[$i]['datetime'])).'</td>';
      $body .= '<td>'.$result[$i]['title'].'</td>';
      $body .= '<td>';
      $body .= '<form style="display:inline;" action="commit.php" method="POST">';
      $body .= '<input type="hidden" name="id" value="'.$result[$i]['id'].'">';
      $body .= '<input type="hidden" name="checked" value="'.$checked.'">';
      $body .= '<input type="hidden" name="status" value="'.$result[$i]['status'].'">';
      $body .= '<input type="hidden" name="answerer" value="'.$answerer.'">';
      $body .= '<input type="hidden" name="owner" value="'.$owner.'">';
      $body .= '<input type="hidden" name="secret" value="'.$result[$i]['secret'].'">';
      $body .= '<input type="submit" class="btn btn-';
      if ($answerer == 1 && $checked == 0 && $result[$i]['status'] != 2) {
        $body .= 'primary" value="確認';
      }else{
        $body .= 'default" value="表示';
      }
